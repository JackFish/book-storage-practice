/**
 * Created by gwonseongbong on 8/17/16.
 */
...
@reduxForm(
    ...
    state => ({
        ...
        page: state.lawyer.lawyerList.number ? state.lawyer.lawyerList.number + 1 : 1,
        lawyer: state.lawyer.lawyer,
        workStateTypeCode: state.lawyer.workStateTypeCode
    }),
    {pushState, create, update, setSortLawyerLawFirm}
)
class LawyerForm extends Component {
    static propTypes = {}

    handleSort(sortedArray) {
        const {values, setSortLawyerLawFirm} = this.props;
        setSortLawyerLawFirm(values, sortedArray);
    }

    addLawyerLawFirmList(){
        const {fields: {lawyerLawFirmList}, workStateTypeCode, values} = this.props;
        const defaultWorkState = workStateTypeCode[0].code;
        const lastSortOrder = values.lawyerLawFirmList.length+1;
        lawyerLawFirmList.addField({lawFirm:{name:""},workState:defaultWorkState,sortOrder:lastSortOrder});
    }

    removeLawyerLawFirmList(index){
        const {fields: {lawyerLawFirmList}} = this.props;
        if(lawyerLawFirmList.length === 1){
            return
        }
        lawyerLawFirmList.removeField(index);
    }

    componentDidMount(){
        const {fields: {lawyerLawFirmList}, workStateTypeCode, formType} = this.props;
        const defaultWorkState = workStateTypeCode[0].code;
        const lastSortOrder = 1;
        if(formType === "create"){
            lawyerLawFirmList.addField({lawFirm:{name:""},workState:defaultWorkState,sortOrder:lastSortOrder});
        }
    }
    ...
    render() {
        ...
        const {loading, workStateTypeCode, pushState, create, update, page} = this.props;
        const isImageGroup = imageGroup && imageGroup.mainImageThumbUrl && imageGroup.mainImageThumbUrl.value;
        const isPreviewUrl = file && file.file && file.file.value && file.previewUrl.value;
        const isLawyerImage = isImageGroup || isPreviewUrl;
