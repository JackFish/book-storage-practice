...
import {loadSearchStatus, search} from 'redux/reducers/bookReport';
...
@connect(
    state => ({
        ...
        searchStatus: state.bookReport.searchStatus
    }),
    {pushState, search}
)
export default class BookReport extends Component {
    ...
    onClickSearchButton(){
        ...
        search(term, code);
        pushState('/bookReport?term=' + term + '&type=' + code);
    }
    ...
    render() {
        ...
        const title = (
            <div className="clearfix">
                ...
                {
                    (!path || path === 'page/:page') &&
                    <Button disabled={loading} className="pull-right" onClick={() => pushState('/bookReport/form')}>
                        등록
                    </Button>
                }
